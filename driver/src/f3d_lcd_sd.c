...
void f3d_lcd_sd_interface_init(void) {
 /* vvvvvvvvvvv pin initialization for the LCD goes here vvvvvvvvvv*/ 
  ...
  // Initialize ports 9 (to RS), 10 (to RESET), 11 (to BLK), 12 (to LCD_CS)
  ...
  // Initialize AF pins 13 (SCLK), 14 (MISO), 15 (MOSI)
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;

	GPIO_Init(GPIOB, &GPIO_InitStructure);

  // Set alternate functions for pins 13 - 15
...
void f3d_lcd_drawBar(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint16_t color) {
  if ((x >= ST7735_width) || (y >= ST7735_height)) return;

  
  uint16_t to_draw[width];
  
  uint8_t i;
  for (i = 0; i < width; i++) to_draw[i] = color;


  f3d_lcd_setAddrWindow(x, y, x + width, y + height, MADCTLGRAPHICS);
  for (i = 0; i < height; i++) {
    f3d_lcd_pushColor(to_draw, width);
  }
}
